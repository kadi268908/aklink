...
async def get_file_ids(client: Client, chat_id: int, id: int) -> Optional[FileId]:
    try:
        message = await asyncio.wait_for(client.get_messages(chat_id, id), timeout=30)
    except asyncio.TimeoutError:
        logging.error(f"Timeout getting message {id} from chat {chat_id}")
        raise
    except Exception as e:
        logging.error(f"Error getting message {id}: {e}")
        raise
    
    if message is None or (hasattr(message, 'empty') and message.empty):
        raise FIleNotFound
